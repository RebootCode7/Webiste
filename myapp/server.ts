...
app.post('/api/admin/login', async (req, res) => {  
  ...
  // It's highly recommended to store a hashed password, not plain text.
  // For this example, we'll assume 'adminPassword' is a bcrypt hash.
  // Default credentials for demonstration if not set in db.json
  const dbUser = db.settings?.adminUsername || 'admin';
  // Default password 'rebootcode' hashed with bcrypt (salt rounds: 10)
  const dbPassHash = db.settings?.adminPassword || '$2b$10$f.73TjS7h6d8G4j/L.pE..e/w2I3Cg5FzX.E/w2I3Cg5FzX.E/w2I';

  const isPasswordValid = await bcrypt.compare(password, dbPassHash);

  if (username === dbUser && isPasswordValid) {
    // Create a secure JWT token instead of a static string
    const token = jwt.sign({ username: dbUser, role: 'admin' }, JWT_SECRET || 'default-secret-key-please-change', { expiresIn: '1h' });
...
async function startServer() {
  ...
  server.on('error', (err: NodeJS.ErrnoException) => {
    if (err.code === 'EADDRINUSE') {
      console.error(`Port ${PORT} is already in use. Please stop the process using this port or set PORT=anotherPort.`);
      process.exit(1);
    }
